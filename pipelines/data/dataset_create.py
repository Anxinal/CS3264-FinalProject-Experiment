...
def main():
    ...
    neg_data = np.load(NEGATIVE_SAMPLES_NPZ)
    ...
    print(f"Before variance filter - Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}")

    selector = VarianceThreshold(threshold=0.0)
    X_train = selector.fit_transform(X_train)
    X_val = selector.transform(X_val)
    X_test = selector.transform(X_test)
    print(f"After variance filter  - Train: {X_train.shape}, Val: {X_val.shape}, Test: {X_test.shape}")
    ...
    print(f"Saved dataset splits to: {X_TRAIN_NPY.parent}")
