...
def main():
    ...
    for _, row in df.iterrows():
        ...
        positive_samples.append((sum_up, 1))

    X_pos = np.array([s[0] for s in positive_samples])
    y_pos = np.array([s[1] for s in positive_samples])
    np.savez(POSITIVE_ADD_SAMPLES_NPZ, X=X_pos, y=y_pos)
    print(f"Positive additive sample count: {len(positive_samples)}")
    print(f"Saved to: {POSITIVE_ADD_SAMPLES_NPZ}")
